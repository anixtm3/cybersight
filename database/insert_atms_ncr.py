...
with open(r'C:\Users\SAINA20\Downloads\delhi ncr.geojson', 'r', encoding='utf-8') as f:
    data = json.load(f)
...
for i, feature in enumerate(data['features']):
    ...
    atm_id = f"NCR{str(i + 1).zfill(5)}"

    try:
        cur.execute("""
                    INSERT INTO atm_locations (atm_id, bank_name, address, district, state, location)
                    VALUES (%s, %s, %s, %s, %s, ST_SetSRID(ST_MakePoint(%s, %s), 4326)) ON CONFLICT (atm_id) DO NOTHING
                    """, (
                        atm_id,
                        bank_name,
