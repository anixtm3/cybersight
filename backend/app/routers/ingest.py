...
from app.models.complaint import Complaint, KeywordFraudMap, Prediction, AlertDispatchLog, MuleAccount
...
FORBIDDEN_SQL = [";", "--", "'", '"', "DROP", "SELECT", "UNION", "INSERT", "DELETE", "UPDATE"]
...
def flag_mule_on_blockchain(account_id: str, risk_score: int, reason: str):
    try:
...
def ingest_complaint(complaint_data: IngestRequest, request: Request, db: Session = Depends(get_db)):

    # 1. SQL Validation
    validate_text(complaint_data.complaint_text)
    validate_text(complaint_data.victim_district)
    validate_text(complaint_data.victim_state)
